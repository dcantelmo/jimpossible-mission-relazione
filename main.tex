...
La flessibilità di questa soluzione ha permesso di estendere questo comportamente anche per la sostituzione delle immagini rappresentanti i collezionabili.
...
\end{figure}
\newline
Lo spritesheete viene suddiviso in righe, che rappresentano le diverse animationi, e colonne, che rappresentano i frame di ogni animazione.
...
\begin{itemize}
  ...
  \item Flipped frames, ovvero frame utilizzati per quando il Player e' direzionato nella parte opposta.
  \item Tipologia di animazione (Enum AnimationMode: LOOP, ONCE)
  \item Durata di ogni frame (in millisecondi)
\end{itemize}
...
\begin{center}
  ...
  \fcolorbox{codepurple}{backcolour}{\parbox{0.9\linewidth}{\small
      ...
      \newline
      {\small\textsf{\textcolor{codepurple}{Si rimanda alla sezione~\ref{sec:damaged_player_animation} in appendice per i dettagli implementativi.}}}
  }}
\end{center}
