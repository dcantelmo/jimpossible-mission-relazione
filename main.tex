...
La classe \textbf{PhysicsState} si occupa di sommare le forze costantie di aggiornare la velocità tenendo conto del \(\Delta t\) calcolato nel game loop,
...
L'adozione del pattern MVC ha contribuito a mantenere una separazione netta tra logica di gioco e interfaccia utente, contribuendo a mantenere il progetto organizzato, nonostante la moltitudine di classi gestite.
Pattern come State hanno facilitato ampiamente la gestione di comportamenti complessi, separando e isolando logiche che altrimenti sarebbero state inserite in lunghe porzioni di codice poco manutenibile e difficilmente estendibile.
L'Observer pattern ha permesso di creare un flusso di dati reattivo tra Model e View, rinforzando ulteriormente la separazione tra i package e favorendo il decoupling.
Infine, è risultato essenziale applicare i principi della programmazione orientata agli oggetti, come astrazione e composizione, sfruttando quindi interfacce, 
ereditarietà e classi astratte per creare entità flessibili e riutilizzabili.
\newpage
\appendix
\section{Appendice}
